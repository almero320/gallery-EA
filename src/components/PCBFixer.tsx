...
const LEVELS: LevelConfig[] = [
  { level: 1, time: 25, blockedRatio: 0.1, lockedCells: 0, label: "NOVICE", pathRandomness: 0.05, minPathLength: 8 },
  { level: 2, time: 22, blockedRatio: 0.15, lockedCells: 0, label: "APPRENTICE", pathRandomness: 0.15, minPathLength: 10 },
  { level: 3, time: 20, blockedRatio: 0.2, lockedCells: 1, label: "TECHNICIAN", pathRandomness: 0.25, minPathLength: 12 },
  { level: 4, time: 18, blockedRatio: 0.25, lockedCells: 1, label: "ENGINEER", pathRandomness: 0.35, minPathLength: 14 },
  { level: 5, time: 16, blockedRatio: 0.3, lockedCells: 2, label: "SPECIALIST", pathRandomness: 0.45, minPathLength: 16 },
  { level: 6, time: 14, blockedRatio: 0.35, lockedCells: 2, label: "EXPERT", pathRandomness: 0.55, minPathLength: 20 },
  { level: 7, time: 12, blockedRatio: 0.4, lockedCells: 3, label: "MASTER", pathRandomness: 0.65, minPathLength: 24 },
  { level: 8, time: 10, blockedRatio: 0.45, lockedCells: 3, label: "GURU", pathRandomness: 0.75, minPathLength: 28 },
  { level: 9, time: 9, blockedRatio: 0.5, lockedCells: 4, label: "LEGEND", pathRandomness: 0.85, minPathLength: 32 },
  { level: 10, time: 8, blockedRatio: 0.55, lockedCells: 4, label: "NIGHTMARE", pathRandomness: 0.9, minPathLength: 36 },
  { level: 11, time: 7, blockedRatio: 0.6, lockedCells: 5, label: "HELL", pathRandomness: 0.95, minPathLength: 40 },
  { level: 12, time: 6, blockedRatio: 0.65, lockedCells: 5, label: "IMPOSSIBLE", pathRandomness: 1.0, minPathLength: 44 },
];
...
export function PCBFixer({
  ...
  useEffect(() => { padARef.current = padA; }, [padA]);
  useEffect(() => { padBRef.current = padB; }, [padB]);
  ...
  }, [grid, gameState, padA, padB, score, streak, highScore, bestStreak, level, timeLeft, wrongTraces]);

  // 🔥 PERBAIKAN UTAMA: Menggunakan document.elementFromPoint untuk akurasi posisi pixel-perfect
  const getCellFromPoint = useCallback(
    (clientX: number, clientY: number): [number, number] | null => {
      const element = document.elementFromPoint(clientX, clientY);
      if (!element) return null;
      
      const cellElement = element.closest("[data-pcb-cell]");
      if (!cellElement) return null;

      const x = parseInt(cellElement.getAttribute("data-x") || "-1", 10);
      const y = parseInt(cellElement.getAttribute("data-y") || "-1", 10);
  ...
  const processDragMove = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (lastPos[0] === x && lastPos[1] === y) return;

      const dx = Math.abs(x - lastPos[0]);
      const dy = Math.abs(y - lastPos[1]);
      if (dx + dy !== 1) return;

      const alreadyInPath = dragPathRef.current.some(([px, py]) => px === x && py === y);
  ...
  const processDragEnd = useCallback(() => {
    ...
    const lastPos = dragPathRef.current[dragPathRef.current.length - 1];
    const reachedB = lastPos && lastPos[0] === padBRef.current[0] && lastPos[1] === padBRef.current[1];
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (target.closest(".pcb-grid-container") !== null) {
        e.preventDefault();
  ...
  const handleNextLevel = () => {
    ...
    setLevel(nextLevel);
    setShowLevelUp(true);
    setTimeout(() => { initBoard(nextLevel); }, 1500);
  };
  ...
  const getCellStyle = (cell: CellType, x: number, y: number): React.CSSProperties => {
    const isInDragPath = dragPath.some(([px, py]) => px === x && py === y);
    const isLastDrag = dragPath.length > 0 && dragPath[dragPath.length - 1][0] === x && dragPath[dragPath.length - 1][1] === y;
    ...
    if (powerOn) { base.backgroundColor = "#32CD32"; } 
    else if (flickerPhase) { base.backgroundColor = Math.random() > 0.5 ? "#32CD32" : "#002200"; } 
    else { base.backgroundColor = "#002200"; }
    ...
    if (cell === "trace" || isInDragPath) {
      if (powerOn) {
        ...
        base.boxShadow = isInDragPath ? "0 0 10px #FFD700, inset 0 0 5px #FFA500" : "inset 0 0 4px #B8860B";
    ...
    if (cell === "padA" || cell === "padB") {
      base.backgroundColor = powerOn ? "#00ff00" : "#228B22";
      base.boxShadow = powerOn ? "0 0 20px #00ff00, inset 0 0 10px #fff" : "none";
  ...
  const getCellContent = (cell: CellType, x: number, y: number) => {
    ...
    if (cell === "padA") return <span className="text-white text-xs font-bold">A</span>;
    if (cell === "padB") return <span className="text-white text-xs font-bold">B</span>;
    if (cell === "locked") return <span className="text-[10px]">🔒</span>;
    if (cell === "blocked")
      return (
        <div className="w-[70%] h-[40%] bg-[#333] border border-[#444] rounded-sm relative pointer-events-none">
    ...
    if (cell === "trace" || isLastDrag) {
      if (powerOn) return <div className="w-[80%] h-[80%] bg-white/90 rounded-sm shadow-[0_0_8px_#fff,0_0_16px_#00ffff] pointer-events-none" />;
      return <div className="w-[80%] h-[80%] rounded-sm bg-[#DAA520] shadow-[inset_0_0_4px_#B8860B] pointer-events-none" style={isLastDrag ? {backgroundColor: "#FF00FF"} : {}} />;
    }
    return null;
  };

  return (
    <div className={`window h-full flex flex-col ${isActive ? "ring-2 ring-[#FF00FF]" : ""}`} onClick={setActive}>
      {/* Title Bar */}
      <div className="title-bar flex items-center justify-between cursor-move shrink-0">
        <div className="flex items-center gap-2">
          <span>🔧</span>
          <span className="hidden sm:inline">PCB-FIXER v3.0 — LVL {level} [{config.label}]</span>
          <span className="sm:hidden">PCB L{level}</span>
        </div>
        <div className="flex gap-1">
          <button type="button" onClick={(e) => { e.stopPropagation(); onMinimize(); }} className="hidden sm:inline px-2 hover:bg-white/20">🗕</button>
          <button type="button" onClick={(e) => { e.stopPropagation(); onMaximize(); }} className="hidden sm:inline px-2 hover:bg-white/20">🗖</button>
          <button type="button" onClick={(e) => { e.stopPropagation(); onClose(); }} className="px-2 hover:bg-red-500 hover:text-white">✕</button>
        </div>
      </div>

      {/* Game Area */}
      <div ref={gameAreaRef} className="flex-1 p-2 sm:p-3 bg-[#001100] flex flex-col items-center justify-start overflow-hidden">
        {/* Stats */}
        ...
        <div className="w-full max-w-[420px] flex items-center justify-between text-[10px] mb-1 shrink-0">
          ...
          <span className={`${isDragging ? "text-[#FF00FF] animate-pulse" : "text-[#888]"}`}>{hintText}</span>
        </div>

        {/* Timer */}
        <div className="w-full max-w-[420px] mb-2 shrink-0">
          <div className="flex items-center justify-between mb-0.5">
            <span className={`text-[10px] font-bold ${timeLeft <= 5 ? "text-[#FF0040] animate-pulse" : "text-[#FF00FF]"}`}>⏱️ {timeLeft}s</span>
            {timeLeft <= 5 && gameState === "playing" && (
              <span className="text-[#FF0040] text-[10px] font-bold animate-pulse">⚠️ HURRY!</span>
            )}
          </div>
          <div className="w-full h-3 rounded border-2 overflow-hidden" style={{ borderColor: timeLeft <= 5 ? "#FF0040" : "#FF00FF", background: "#1a1a1a", boxShadow: timeLeft <= 5 ? "0 0 10px #FF0040" : "none" }}>
            <div className="h-full transition-all duration-1000 ease-linear" style={{ width: `${(timeLeft / config.time) * 100}%`, background: timeLeft <= 5 ? "linear-gradient(90deg, #FF0040, #FF6666)" : timeLeft <= config.time * 0.25 ? "linear-gradient(90deg, #FFD700, #FFAA00)" : "linear-gradient(90deg, #FF00FF, #C44BFF)", boxShadow: timeLeft <= 5 ? "0 0 15px #FF0040" : "none" }} />
          </div>
        </div>
        ...
        <div
          ...
          style={{
            ...
            background: powerOn ? "#32CD32" : flickerPhase ? "#1a5a1a" : "#001100",
            borderColor: powerOn ? "#32CD32" : timeLeft <= 5 && gameState === "playing" ? "#FF0040" : "#FF00FF30",
            boxShadow: powerOn ? "0 0 30px #32CD32, inset 0 0 15px #228B22" : timeLeft <= 5 && gameState === "playing" ? "0 0 15px #FF0040" : "none",
            touchAction: "none",
        ...
        {touchFeedback.show && (
          <div
            ...
            style={{ left: touchFeedback.x - 16, top: touchFeedback.y - 16, transform: "scale(1)" }}
          />
        )}
        ...
        <div className="mt-2 flex gap-2 flex-wrap justify-center shrink-0">
          {gameState === "idle" || gameState === "won" || gameState === "lost" ? (
            <button onClick={() => initBoard()} className="nes-btn is-primary text-[10px] sm:text-xs px-3 py-1">
              {gameState === "idle" ? "START GAME" : "PLAY AGAIN"}
            </button>
          ) : (
            <button onClick={() => initBoard()} className="nes-btn is-error text-[10px] sm:text-xs px-3 py-1">RESTART</button>
          )}
          {gameState === "won" && (
            <button onClick={handleNextLevel} className="nes-btn is-success text-[10px] sm:text-xs px-3 py-1 animate-bounce">NEXT LEVEL →</button>
          )}
        </div>
        ...
        {gameState === "idle" && (
          <div className="text-center mt-1 text-[#FFD700] text-[10px] max-w-[420px] shrink-0 leading-tight">
            ...
            <p className="text-[#FF0040]">⚠️ One mistake = start over. No mercy.</p>
          </div>
        )}
      </div>
      ...
      {showLevelUp && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[200]">
          <div className="text-center">
            ...
            <h2 className="text-[#32CD32] text-xl font-bold tracking-widest animate-pulse">LEVEL {level} UNLOCKED</h2>
            <p className="text-[#FFD700] text-xs mt-1">{getLevelConfig(level).label}</p>
            <p className="text-[#FF0040] text-[10px] mt-1">{getLevelConfig(level).time}s | {Math.round(getLevelConfig(level).blockedRatio * 100)}% blocked</p>
          </div>
        </div>
      )}
      ...
      {gameState === "won" && !flickerPhase && !showLevelUp && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[200] p-4">
          <div className="nes-container is-rounded is-dark !bg-[#002200] !border-[#32CD32] max-w-xs w-full text-center p-4">
            ...
            <h2 className="text-[#32CD32] text-base font-bold tracking-wider">CIRCUIT STABLE.</h2>
            <h2 className="text-[#00ffff] text-base font-bold mb-2 tracking-wider">SYSTEM ONLINE.</h2>
            ...
            <div className="flex gap-2 justify-center mt-3">
              <button onClick={handleNextLevel} className="nes-btn is-success text-[10px] px-3 py-1">NEXT →</button>
              <button onClick={handleRestart} className="nes-btn is-error text-[10px] px-3 py-1">RESET</button>
            </div>
          </div>
        </div>
      )}
      ...
      {gameState === "lost" && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[200] p-4">
          <div className="nes-container is-rounded is-dark !bg-[#220000] !border-[#FF0040] max-w-xs w-full text-center p-4">
            <div className="text-3xl mb-1">💥</div>
            <h2 className="text-[#FF0040] text-base font-bold mb-2 tracking-wider">CIRCUIT FAILED.</h2>
            <p className="text-[#FFD700] text-[10px] mb-1">Time ran out before connection.</p>
            <p className="text-[#888] text-[10px] mb-1">Level {level} — {config.label}</p>
            <p className="text-[#FF00FF] text-[10px] mb-2">Score: {score} | Best: {bestStreak}</p>
            <div className="flex gap-2 justify-center">
              <button onClick={() => initBoard()} className="nes-btn is-primary text-[10px] px-3 py-1">RETRY</button>
              <button onClick={handleRestart} className="nes-btn is-error text-[10px] px-3 py-1">RESET ALL</button>
            </div>
          </div>
        </div>
      )}
